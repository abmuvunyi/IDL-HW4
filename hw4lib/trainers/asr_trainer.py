            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                if self.ctc_weight > 0 and targets_golden is not None:
                    ...
                    for b_idx in range(targets_golden.size(0)):
                        length_val = transcript_lengths[b_idx].item()
                        unpadded = targets_golden[b_idx, :length_val]
                        target_list.append(unpadded)
                        target_len_list.append(length_val)

                    targets_1d = torch.cat(target_list)
    ...
    def _validate_epoch(self, dataloader):
        ...
        for r in results:
            if 'target' in r:
                ...
                # If no target is present (like test data),
                # use blank or skip
                references.append("")
